...
exports.bookinstance_detail = function(req, res) {
    ...
    .exec(function (err, bookinstance){
        if (err) {return next(err);}
        if (bookinstance == null){ // no results found.
...
exports.bookinstance_delete_get = function(req, res, next) {
    BookInstance.findById(req.params.id).exec(function(err, results){
        if (err){return next(err);}
        if (results == null){
            res.status(404).send('This book instance does not exist');
        }
...
exports.bookinstance_delete_post = function(req, res) {
    ...
    BookInstance.findByIdAndDelete(req.params.id).exec(function(err, results){
        if (err){return next(err);}
        if (results == null){
            res.status(404).send('This book instance does not exist');
        }
